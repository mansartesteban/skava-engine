...
import { CommandMouseMove, CommandTouchMaintain, CommandToggle, CommandOnce, CommandSwipe, CommandHold } from "./Core/Commands.es.js";
...
class Controls {
  constructor(pointerLock = false) {
    ...
    __publicField(this, "touchStart", new Vector2());
    __publicField(this, "touch", new Vector2());
  ...
  update(deltaTime) {
    let holdCommands = this.commands.filter(
      (command) => command instanceof CommandHold && this.stack.includes(command.key)
    );
...
onKeyUp_fn = function(e) {
  ...
  let toggleCommands = this.commands.filter(
    (command) => command instanceof CommandToggle && e.code === command.key && command.executed
  );
  ...
  let onceCommands = this.commands.filter(
    (command) => command instanceof CommandOnce && e.code === command.key && command.executed
  );
...
//# sourceMappingURL=Controls-DExq5fSR.mjs.map
