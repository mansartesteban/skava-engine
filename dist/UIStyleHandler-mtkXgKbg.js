...
const Utils_Object = require("./Utils/Object.cjs.js");
class UIComponent extends Entity.Entity {
  constructor(...components) {
    ...
    __publicField(this, "datas", {
      origin: new Vector2.Vector2(),
      defaultSlot: null
    });
    __publicField(this, "slots", /* @__PURE__ */ new Map());
    __publicField(this, "eventObserver", new Entity.Observer({
      click: "click"
    }));
  ...
  addRenderer(renderer) {
    if (!(renderer instanceof UIRenderer)) {
      throw new WrongInstanceError.WrongInstanceError(renderer, UIRenderer);
    }
...
class UIRenderer {
  ...
  render() {
    throw new ImplementError.ImplementError("render", "UIRenderer");
  }
}
const _UIStyle = class _UIStyle extends Vector2.Component {
  ...
  setDefaultStyle(style) {
    ...
    Object.keys(Utils_Object.getProperties(style)).forEach((prop) => {
      if (prop !== "inherit") {
  ...
  setStyle(style, overwrite = true, isDefault = false) {
    ...
    if (style instanceof _UIStyle) {
      styleValues = Utils_Object.getProperties(style);
    }
...
class UIStyleHandler {
  constructor(uiStyle) {
    ...
    __publicField(this, "datas", {
      size: new Vector2.Vector2(),
      margin: [0, 0, 0, 0]
    });
    if (!(uiStyle instanceof UIStyle)) {
      throw new WrongInstanceError.WrongInstanceError(uiStyle, UIStyle);
    }
    this.style = uiStyle;
  ...
  calculateSize(parentTransform) {
    ...
    return new Vector2.Vector2(Math.max(0, x), Math.max(0, y));
  }
  getNextFreePosition(component, index) {
    let nextFreePosition = new Vector2.Vector2();
    if (component) {
...
exports.UIStyleHandler = UIStyleHandler;
//# sourceMappingURL=UIStyleHandler-mtkXgKbg.js.map
